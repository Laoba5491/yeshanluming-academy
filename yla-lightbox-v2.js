(() => {
  ...
  function ensureOverlay() {
    ...
    image.addEventListener('click', (event) => { event.stopPropagation(); setMode(mode === 'fit' ? 'actual' : 'fit'); });
    closeButton.addEventListener('click', close);
  }
  function setMode(nextMode) {
    mode = nextMode; overlay.classList.toggle('fit',mode === 'fit'); overlay.classList.toggle('actual',mode === 'actual');
    hint.textContent = mode === 'fit' ? '再次點擊查看原始像素 · Click again for actual size' : '原始像素模式 · 再次點擊返回全圖 · Actual size · Click again to fit';
  ...
  document.addEventListener('keydown',(event)=>{if(event.key==='Escape')close();if((event.key==='Enter'||event.key===' ')&&overlay&&overlay.classList.contains('open')){event.preventDefault();setMode(mode==='fit'?'actual':'fit')}});
